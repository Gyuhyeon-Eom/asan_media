...
BROADCASTS = {
    '전국노래자랑': {
        ...
        'sites': ['신정호'],
        'pre_range': ('2025-04-11', '2025-06-07'),
        'post_range': ('2025-06-08', '2025-09-30'),
    },
    '전현무계획2': {
...
def run_causal_impact(name, info):
    ...
    # 누락 기간(05-06)이 있으면 0으로 채워짐 → 제거
    # 전국노래자랑: 04-11~06-07(pre) + 07-01~09-30(post), 05-01~06-07은 데이터 없어 0
    # 이 경우 pre 기간에서 실제 데이터 있는 04월만 사용
    if name == '전국노래자랑':
        # pre: 04-11~04-30 실제 데이터만, post: 07-01~09-30
        mask_pre = (ci_data.index >= '2025-04-11') & (ci_data.index <= '2025-04-30')
        mask_post = (ci_data.index >= '2025-07-01') & (ci_data.index <= '2025-09-30')
        ci_data = ci_data[mask_pre | mask_post]
        bdate = pd.Timestamp('2025-07-01')  # 실질적 개입 시점을 데이터 있는 첫날로
...
def run_yoy_did(name, info):
    if name == '뛰어야산다2':
        ...
        # 계절 비교 형태: 방송 전(04월) vs 방송 후(07~09월) 처치/대조 변화
        treated = info['sites']
        control = get_control_sites(treated)
        if not control:
            return None
        ts = daily_pivot.copy().sort_index()
        pre = ts[(ts.index >= '2025-04-11') & (ts.index <= '2025-04-30')]
        post = ts[(ts.index >= '2025-07-01') & (ts.index <= '2025-09-30')]
        ...
        result = {
            ...
            'did_effect': did, 'significant': abs(did) > 0.05,
            'note': '일평균 기준, 전년 동기 없어 방송 전후 계절 비교',
        }
...
def run_covariate_did(name, info):
    ...
    if is_all:
        ...
        # 전국노래자랑: 05~06월 데이터 없으므로 0인 날 제거
        if name == '전국노래자랑':
            row_sum = ts.sum(axis=1)
            ts = ts[row_sum > 0]
...
if nr.get('yoy'):
    ...
    w = 0.35
    ax.bar(x - w/2, pre_vals, w, label='방송 전(04월 일평균)', color='#90CAF9', edgecolor='#333', lw=0.5)
    ax.bar(x + w/2, post_vals, w, label='방송 후(07~09월 일평균)', color='#1565C0', edgecolor='#333', lw=0.5)
...
데이터: T맵 내비게이션 (2025.01~2026.03)</p>
...
<h2>5. 한계</h2>
<ul>
<li>2025년 5~6월, 10월 T맵 데이터 부재. 전국노래자랑은 방송 전 20일(4월)만으로 사전 기간 짧음.</li>
